...
from ..errors.custom_error import InvalidInputsError, ServiceObjectLogicError
...
class ServiceWithResult(Service, abc.ABC):
    ...
    def stop_process(self):
        if bool(self._errors):
            raise ServiceObjectLogicError(
                errors_dict=self._errors, response_status=self.response_status or 400
            )
        else:
            return self
...
class ServiceOutcome:
    ...
    def __init__(
        self, service_object, service_object_attributes=None, service_object_files=None
    ):
        self._errors = {}
        self._result = None
        self._response_status = None
        self._outcome = self.execute(
            service_object, service_object_attributes, service_object_files
        )

    def execute(self, service_object, service_object_attributes, service_object_files):
        outcome = service_object.execute(
            service_object_attributes, service_object_files
        )
        self._response_status = outcome.response_status
        if bool(outcome.errors):
            response_status = (
                self.response_status if hasattr(self, "response_status") else 400
            )
            self._errors = outcome.errors
            raise ServiceObjectLogicError(
                errors_dict=outcome.errors, response_status=response_status
            )
        else:
